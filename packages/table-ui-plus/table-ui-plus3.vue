...
<template>
    <div :style="Div_width" class="box-flex" ref="content_box">
        <div class="flex-top" ref="collapse_box" v-if="Table_collapse_show">
            <!--表头控制区域-->
            ...
            setViewHeight: function (viewHeight) {
                ...
                if (this.autoFixHeight === true) {
                    ...
                    if (this.$refs.collapse_box && this.$refs.page_box) {
                        this.$nextTick(() => {
                            let $navHead = document.querySelector('[data-ui-name="nav-head"]');
                            let $selectorUi = document.querySelector('[data-ui-name="selector-ui"]');
                            this.Table_maxHeight = this.viewHeight - (this.$refs.collapse_box.offsetHeight + this.$refs.page_box.offsetHeight + ($navHead ? $navHead.offsetHeight : 0) + ($selectorUi ? $selectorUi.offsetHeight : 0) + 20);
                        });
                    }
                }
